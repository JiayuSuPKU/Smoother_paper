...
inputFiles <- './GSM5028439_ME11_H3K4me3_20um.fragments.tsv.gz'
sampleNames <- 'ME11_H3K4me3_20um'
...
filter.matrix = TRUE
slice = "slice1"
image <- Read10X_Image(image.dir = file.path(data.dir, "GSM5028439_spatial"), 
                       filter.matrix = filter.matrix)
meta.data.spatial <- meta.data[paste('ME11_H3K4me3_20um#', row.names(image@coordinates), '-1', sep=""), ]
...
save.image("H3K4me3.20um.RData")


load("H3K4me3.20um.RData")
...
write.csv(all_gene_score, file = "processed/H3K4me3.20um.all_gene_score.csv")
var_gene_score <- all_gene_score[order(rowVars(as.matrix(all_gene_score)),decreasing = T)[1:500],]
write.csv(var_gene_score, file = "processed/H3K4me3.20um.var_gene_score.csv")
# write.csv(gene_score, file = "processed/H3K4me3.20um.gene_score.csv")

write.csv(image@coordinates[,4:5], file = "processed/H3K4me3.20um.coord.csv")
# write.csv(log2(assays(marker_peak_matrix)$PeakMatrix %>% as.matrix +1), file = "processed/H3K4me3.20um.peak_score.csv", row.names = FALSE)
